...
<body>
<!-- 컨텐츠만 바꿀때는 enctype 필요없음. 이번에는 파일까지 바꾸기 위해서 선언 -->
...
규격 : <input type = "number" name="prodCapacity" min = "10" value="${goodsCommand.prodCapacity }"/>
...
배송비 : <input type= "number" name = "prodDelFee" min = "0" step ="500" value="${goodsCommand.prodDelFee }">
		<form:errors path="prodDelFee"/><br />
추천여부 : 
	<input type ="radio" name = "recommend" value="Y" <c:if test="${goodsCommand.recommend =='Y' }">checked</c:if>/>추천
	
	<input type ="radio" name = "recommend" value="N" <c:if test="${goodsCommand.recommend =='N' }">checked</c:if>/>비추천<br />
카테고리 : <select name="ctgr">
			...
			<option value="car"<c:if test="${goodsCommand.ctgr =='car'}">selected</c:if>>자동차용품</option>
		</select> <br />
상세내용 : <br /><textarea rows="5" cols="50" name="prodDetail">${goodsCommand.prodDetail }</textarea>
